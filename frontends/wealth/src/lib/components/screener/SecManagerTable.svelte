...
{#if data.managers.length === 0}
	<div class="mt-empty"><p>No managers found. Try adjusting your filters.</p></div>
{:else}
	<div class="mt-summary">
		<span>{formatNumber(data.total_count, 0)} managers</span>
		<span class="mt-summary__page">Page {data.page} of {totalPages}</span>
	</div>
					...
					<tr class="mt-row" class:mt-row--no-cik={!mgr.cik} onclick={() => mgr.cik && onSelect(mgr.cik, mgr.firm_name)}>
						...
						<td class="mt-td mt-td--name">
							<div class="mt-name-cell">
								<div class="mt-avatar">
									<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 14V3h5v3h5v8H2zm1-1h3V4H3v9zm4 0h3V7H7v6z" fill="currentColor" opacity="0.5"/></svg>
								</div>
								<button class="mt-name-btn" onclick={(e: MouseEvent) => { e.stopPropagation(); mgr.cik && onDetail(mgr.cik); }} title="View detail">
									{mgr.firm_name}
								</button>
							</div>
						</td>
						<td class="mt-td mt-td--entity">{mgr.registration_status ?? "\u2014"}</td>
						<td class="mt-td mt-td--state">{mgr.state ?? "\u2014"}</td>
						<td class="mt-td mt-td--aum">{mgr.aum_total != null ? `$${formatNumber(mgr.aum_total, 0)}` : "\u2014"}</td>
...
<style>
	.mt-empty { padding: 48px 24px; text-align: center; color: #62748e; font-size: 14px; }
	.mt-summary { display: flex; align-items: center; justify-content: space-between; padding: 12px 24px; font-size: 13px; color: #62748e; }
	.mt-table-wrap { overflow-x: auto; }
	.mt-table { width: 100%; border-collapse: collapse; }
	.mt-th { padding: 12px 24px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.1px; color: #62748e; text-align: left; background: rgba(248, 250, 252, 0.5); border-bottom: 1px solid #f1f5f9; }
	.mt-th--right { text-align: right; }
	.mt-th--center { text-align: center; }
	.mt-row { cursor: pointer; border-bottom: 1px solid #f1f5f9; transition: background 100ms ease; height: 64px; }
	.mt-row:hover { background: rgba(248, 250, 252, 0.5); }
	.mt-row:last-child { border-bottom: none; }
	.mt-row--no-cik { cursor: default; opacity: 0.5; }
	.mt-td { padding: 16px 24px; font-size: 14px; vertical-align: middle; }
	.mt-td--name { min-width: 280px; }
	.mt-name-cell { display: flex; align-items: center; gap: 12px; }
	.mt-avatar { width: 32px; height: 32px; border-radius: 10px; background: #f1f5f9; display: flex; align-items: center; justify-content: center; color: #62748e; flex-shrink: 0; }
	.mt-name-btn { background: none; border: none; padding: 0; font-size: 14px; font-weight: 700; color: #1d293d; cursor: pointer; text-align: left; }
	.mt-name-btn:hover { text-decoration: underline; }
	.mt-td--entity, .mt-td--state { color: #62748e; font-weight: 500; }
	.mt-td--funds { text-align: center; }
	.mt-fund-tags { display: flex; gap: 4px; flex-wrap: wrap; justify-content: center; }
	.mt-fund-tag { display: inline-block; padding: 2px 6px; font-size: 10px; font-weight: 700; border-radius: 4px; line-height: 1.2; letter-spacing: 0.3px; }
