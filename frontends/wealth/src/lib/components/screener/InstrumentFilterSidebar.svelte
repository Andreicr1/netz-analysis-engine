...
<script lang="ts">
	...
	type FacetKey = "geographies" | "domiciles" | "currencies" | "strategies" | "asset_classes";
	...
	const ALL_FACET_KEYS: FacetKey[] = ["geographies", "domiciles", "currencies", "strategies", "asset_classes"];
	...
	function facetTooltip(key: FacetKey): string {
		// Find which chips have this facet
	...
	function getFacetValue(key: FacetKey): string | null {
		switch (key) {
			case "geographies": return searchGeography;
			case "domiciles": return searchDomicile;
			case "currencies": return searchCurrency;
			default: return null;
		}
	}
	...
	function setFacetValue(key: FacetKey, value: string | null) {
		switch (key) {
			case "geographies": searchGeography = value; break;
			case "domiciles": searchDomicile = value; break;
			case "currencies": searchCurrency = value; break;
		}
		applySearchFilters();
	}
		...
		{#if items.length > 0}
			{#if active}
				<div class="scr-field" transition:slide={{ duration: 200 }}>
					<label class="scr-label" for="search-{facetKey}">{FACET_LABELS[facetKey]}</label>
					<select
						id="search-{facetKey}"
						class="scr-select"
						value={getFacetValue(facetKey)}
						onchange={(e) => setFacetValue(facetKey, (e.target as HTMLSelectElement).value || null)}
					>
			...
			{:else}
				<div class="scr-field scr-field--disabled" title={facetTooltip(facetKey)} transition:slide={{ duration: 200 }}>
					<label class="scr-label scr-label--disabled" for="search-{facetKey}-dis">{FACET_LABELS[facetKey]}</label>
					<select id="search-{facetKey}-dis" class="scr-select scr-select--disabled" disabled>
						<option>All</option>
					</select>
				</div>
			{/if}
