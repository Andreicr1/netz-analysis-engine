<!--
  AllocationView — strategic, tactical, and effective views with profile selector.
...
<script lang="ts">
	...
	let strategic = $state<AllocationRow[]>([]);
	let tactical = $state<TacticalPosition[]>([]);
	let effective = $state<EffectiveRow[]>([]);
	let loading = $state(true);

	// svelte-ignore state_referenced_locally
	let activeProfile = $state("moderate");
	...
	async function fetchData(profile: string) {
		loading = true;
		try {
			...
			strategic = (s.status === "fulfilled" ? s.value : []) as AllocationRow[];
			tactical = (t.status === "fulfilled" ? t.value : []) as TacticalPosition[];
			effective = (e.status === "fulfilled" ? e.value : []) as EffectiveRow[];
		} finally {
			loading = false;
		}
	}
					...
					{#if simulationResult}
						<div class="rounded-lg border {simulationResult.within_limit ? 'border-(--netz-border)' : 'border-(--netz-status-error)'} bg-(--netz-surface-elevated) p-5">
							<div class="mb-3 flex items-center justify-between">
								...
								{:else}
									<span class="inline-flex items-center gap-1 rounded-full bg-(--netz-status-error)/15 px-2 py-0.5 text-xs font-medium text-(--netz-status-error)">Exceeds CVaR Limit — Submit Blocked</span>
								{/if}
							</div>
							<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
								...
								<MetricCard
									...
									sublabel="of limit"
									status={simulationResult.cvar_utilization_pct != null && parseFloat(String(simulationResult.cvar_utilization_pct)) > 1 ? "breach" : simulationResult.cvar_utilization_pct != null && parseFloat(String(simulationResult.cvar_utilization_pct)) > 0.8 ? "warn" : undefined}
								/>
								<MetricCard label="CVaR Delta vs Current" value={fmtSimBps(simulationResult.cvar_delta_vs_current)} sublabel="change from current" />
						...
						<div class="grid gap-4 lg:grid-cols-2">
							<div class="rounded-lg border border-(--netz-border) bg-(--netz-surface-elevated) p-5">
								<div class="mb-4 flex items-center justify-between">
									...
									<span class="text-xs text-(--netz-text-muted)">Total: {formatPercent(totalWeight, 1, "en-US")}</span>
								</div>
								<div class="space-y-3">
									{#each effective as row (row.block)}
										<div class="flex items-center justify-between">
											...
											<div class="flex items-center gap-2 text-xs">
												<span class="text-(--netz-text-muted)">{formatPercent(row.strategic_weight, 1, "en-US")}</span>
