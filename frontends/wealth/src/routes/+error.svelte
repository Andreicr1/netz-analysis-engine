...
{:else if page.status === 404}
	<div class="flex h-full items-center justify-center">
		<div class="text-center">
			<h1 class="mb-2 text-4xl font-bold text-(--ii-text-primary)">404</h1>
			<p class="text-(--ii-text-secondary)">Page not found.</p>
			<a
				href="/"
				class="mt-4 inline-block rounded-md bg-(--ii-brand-primary) px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90"
			>
...
{:else}
	<div class="flex h-full items-center justify-center">
		<div class="text-center">
			<h1 class="mb-2 text-4xl font-bold text-(--ii-text-primary)">{page.status}</h1>
			<p class="text-(--ii-text-secondary)">{page.error?.message ?? "Something went wrong."}</p>
			<a
				href="/"
				class="mt-4 inline-block rounded-md bg-(--ii-brand-primary) px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90"
			>
