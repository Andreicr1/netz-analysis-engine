...
<script lang="ts">
  ...
  let largestTacticalTilt = $derived.by(() => {
    ...
    return (
      [...tactical].sort(
        (left, right) => Math.abs(right.overweight) - Math.abs(left.overweight),
      )[0] ?? null
    );
  });
...
<div class="space-y-(--netz-space-section-gap) p-(--netz-space-page-gutter)">
    ...
    {#snippet actions()}
      <div class="flex gap-1">
        {#each profiles as profile (profile)}
          <button
            class="rounded-md px-3 py-1.5 text-xs font-medium capitalize transition-colors {activeProfile ===
            profile
  ...
  <p class="mt-1 text-sm text-(--netz-text-muted)">
    Strategic targets, tactical tilts, and effective exposure for the {activeProfileLabel}
    profile.
  </p>
  ...
  <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
    <MetricCard
      ...
      sublabel="Current allocation lens"
    />
    <MetricCard
      label="Strategic Blocks"
      value={String(activeBlockCount)}
      sublabel="Configured allocation buckets"
    />
    <MetricCard
      label="Tactical Tilts"
      value={String(tacticalCount)}
      sublabel="Live model adjustments"
    />
    <MetricCard
      label="Largest Tilt"
      value={largestTacticalTilt
        ? fmtSignedPercentPoint(largestTacticalTilt.overweight, 1)
        : "—"}
      sublabel={largestTacticalTilt?.block ?? "No tactical positions"}
      status={largestTacticalTilt &&
      Math.abs(largestTacticalTilt.overweight) >= 0.03
        ? "warn"
        : undefined}
    />
  </div>
  ...
  {#if editError}
    ...
    >
      {editError}
      <button class="ml-2 underline" onclick={() => (editError = null)}
        >dismiss</button
      >
    </div>
  {/if}

  <PageTabs {tabs} defaultTab="effective">
    {#snippet children(activeTab)}
      ...
      {#if activeTab === "strategic"}
        {#if strategic.length > 0}
          <div class="grid gap-4 lg:grid-cols-2">
            <div
              class="rounded-lg border border-(--netz-border) bg-(--netz-surface-elevated) p-5"
            >
              <div class="mb-4 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-(--netz-text-primary)">
                  Strategic Weights
                </h3>
                {#if !editing}
                ...
                {:else}
                  <div class="flex items-center gap-2">
                    <span
                      class="text-xs {editValid
                        ? 'text-(--netz-success,#22c55e)'
                        : 'text-(--netz-danger,#ef4444)'}"
                    >
                      Total: {fmtPercentPoint(editTotal)}
                      {editValid
                        ? "✓"
                        : `(${fmtSignedPercentPoint(editTotal - 100)})`}
                    </span>
              ...
              <div class="space-y-3">
                {#each strategic as row (row.block)}
                  <div class="flex items-center justify-between">
                    <span class="text-sm text-(--netz-text-primary)"
                      >{row.block}</span
                    >
                    <div class="flex items-center gap-2">
                      {#if row.min_weight !== null && row.max_weight !== null}
                        <span class="text-xs text-(--netz-text-muted)">
                          [{formatPercent(
                            ...
                            0,
                            "en-US",
                          )}–{formatPercent(row.max_weight, 0, "en-US")}]
                        </span>
                      {/if}
                      {#if editing}
                        ...
                        {@const belowMin =
                          row.min_weight !== null &&
                          blockWeight < row.min_weight * 100}
                        {@const aboveMax =
                          row.max_weight !== null &&
                          blockWeight > row.max_weight * 100}
                        {@const outOfBounds = belowMin || aboveMax}
                        <Input
                          type="number"
                          class="w-20 text-right {outOfBounds
                            ? 'border-(--netz-danger)'
                            : ''}"
                          bind:value={editWeights[row.block]}
                          min={row.min_weight !== null
                            ? row.min_weight * 100
                            : 0}
                          max={row.max_weight !== null
                            ? row.max_weight * 100
                            : 100}
                          step="0.1"
                          title={outOfBounds
                            ? `Out of bounds: min ${row.min_weight !== null ? row.min_weight * 100 : 0}% — max ${row.max_weight !== null ? row.max_weight * 100 : 100}%`
                            : undefined}
                        />
                        {#if outOfBounds}
                          <span class="text-xs text-(--netz-danger)"
                            >{belowMin ? "↓ min" : "↑ max"}</span
                          >
                        {/if}
                      {:else}
            ...
            <div
              class="rounded-lg border border-(--netz-border) bg-(--netz-surface-elevated) p-5"
            >
              <h3 class="mb-4 text-sm font-semibold text-(--netz-text-primary)">
                Distribution
              </h3>
              <div class="h-64">
        ...
        {:else}
          <EmptyState
            title="No Strategic Allocation"
            message="Strategic allocation has not been configured."
          />
        {/if}
        ...
        {#if simError}
          ...
          >
            {simError}
            <button class="ml-2 underline" onclick={() => (simError = null)}
              >dismiss</button
            >
          </div>
        {/if}
        ...
        {#if simulationResult}
          <div
            class="rounded-lg border {simulationResult.within_limit
              ? 'border-(--netz-border)'
              : 'border-(--netz-status-error)'} bg-(--netz-surface-elevated) p-5"
          >
            <div class="mb-3 flex items-center justify-between">
              <h3 class="text-sm font-semibold text-(--netz-text-primary)">
                CVaR Simulation Preview
              </h3>
              {#if simulationResult.within_limit}
                <span
                  class="inline-flex items-center gap-1 rounded-full bg-(--netz-success,#22c55e)/15 px-2 py-0.5 text-xs font-medium text-(--netz-success,#22c55e)"
                >
                  Within Limit
                </span>
              {:else}
                <span
                  class="inline-flex items-center gap-1 rounded-full bg-(--netz-status-error)/15 px-2 py-0.5 text-xs font-medium text-(--netz-status-error)"
                >
            ...
            {#if simulationResult.warnings.length > 0}
              <div
                class="mt-4 rounded-md border border-(--netz-status-warning,#f59e0b) bg-(--netz-status-warning,#f59e0b)/10 p-3"
              >
                <p
                  class="mb-1 text-xs font-semibold uppercase tracking-wider text-(--netz-status-warning,#f59e0b)"
                >
                  Warnings
                </p>
                <ul class="space-y-1">
                  {#each simulationResult.warnings as warning, i (i)}
                    <li class="text-sm text-(--netz-text-primary)">
                      • {warning}
                    </li>
                  {/each}
                </ul>
            ...
            {#if !simulationResult.within_limit}
              <p class="mt-3 text-sm font-medium text-(--netz-status-error)">
                This allocation exceeds the CVaR limit. Adjust weights before
                submitting.
              </p>
            {/if}
          </div>
      ...
      {#if activeTab === "tactical"}
          ...
          >
            <div class="mb-4 flex items-center justify-between">
              ...
              {#if !editingTactical}
                <Button
                  size="sm"
                  variant="outline"
                  onclick={startEditingTactical}>Edit</Button
                >
              {:else}
                <div class="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onclick={() => (editingTactical = false)}>Cancel</Button
                  >
                  <ActionButton
                    size="sm"
            ...
            <div class="space-y-3">
              {#each tactical as pos (pos.block)}
                <div
                  class="grid gap-3 rounded-(--netz-radius-md) border border-(--netz-border-subtle) bg-(--netz-surface-highlight) px-4 py-3 lg:grid-cols-[minmax(0,1.5fr)_140px_120px] lg:items-center"
                >
                  <div class="space-y-2">
                    <span class="text-sm text-(--netz-text-primary)"
                      >{pos.block}</span
                    ...
                    <div class="space-y-1">
                      <div
                        class="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.08em] text-(--netz-text-muted)"
                      >
                        <span>Conviction</span>
                        <span>{convictionLabel(pos.conviction)}</span>
                      </div>
                      <div
                        class="h-2 overflow-hidden rounded-full bg-(--netz-surface-inset)"
                      >
                        <div
                          class="h-full rounded-full bg-(--netz-border-accent)"
                          style:width={convictionWidth(pos.conviction)}
                        ></div>
                      </div>
                    </div>
                  </div>
                  <div class="text-xs text-(--netz-text-muted)">
                    {#if pos.conviction !== null}
                      Signal strength {formatNumber(pos.conviction, 0, "en-US")}
                      / 5
                    {:else}
                      No conviction score
                    {/if}
                  ...
                  <div class="flex items-center justify-end gap-3">
                    ...
                    {:else}
                      <span
                        class="font-medium {pos.overweight >= 0
                          ? 'text-(--netz-success,#22c55e)'
                          : 'text-(--netz-danger,#ef4444)'}"
                      >
        ...
        {:else}
          <EmptyState
            title="No Tactical Positions"
            message="No active tactical overweights."
          />
        {/if}
      {/if}
      ...
      {#if activeTab === "effective"}
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2 text-xs">
                      ...
                      {#if row.tactical_overweight !== 0}
                        <span
                          class={row.tactical_overweight >= 0
                            ? "text-(--netz-success,#22c55e)"
                            : "text-(--netz-danger,#ef4444)"}
                        >
                          {formatPercent(
                            row.tactical_overweight,
                            1,
                            "en-US",
                            true,
                          )}
        ...
        {:else}
          <EmptyState
            ...
            message="Effective allocation will appear once strategic weights are set."
          />
        {/if}
      {/if}
    {/snippet}
...
<ConsequenceDialog
  ...
  metadata={simulationResult
    ? [
        ...
        {
          label: "Within Limit",
          value: simulationResult.within_limit ? "Yes" : "No",
          emphasis: !simulationResult.within_limit,
        },
      ]
    : []}
  ...
  {#snippet consequenceList()}
    <ul class="space-y-1.5">
      <li class="flex items-start gap-2">
        ...
        <span
          >This will update the strategic allocation for <strong
            class="text-(--netz-text-primary)">{activeProfile}</strong
          ></span
        >
      </li>
      <li class="flex items-start gap-2">
  ...
  {#snippet consequenceList()}
    <ul class="space-y-1.5">
      <li class="flex items-start gap-2">
        <span class="mt-0.5 text-(--netz-warning)">&#9679;</span>
        <span
          >This will update tactical tilts for <strong
            class="text-(--netz-text-primary)">{activeProfile}</strong
          ></span
        >
      </li>
