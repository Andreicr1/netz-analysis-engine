<!--
  ...
  Adapted from admin health page, scoped to wealth workers.
-->
<script lang="ts">
	import {
...
{#if degradedMessage}
	<div
		...
		style="border-color: var(--ii-warning); background-color: color-mix(in srgb, var(--ii-warning) 12%, var(--ii-surface)); color: var(--ii-text-primary);"
		role="alert"
	>
		<p class="font-medium text-(--ii-warning)">Degraded state</p>
...
<SectionCard title="Service Health">
	{#if healthErrors.services}
		<div
			...
			role="alert"
		>
			<p class="font-medium text-(--ii-warning)">Service health unavailable</p>
	...
	{:else}
		<div class="rounded-lg border border-dashed border-(--ii-border) px-4 py-3 text-sm text-(--ii-text-muted)">
...
<SectionCard title="Pipeline Stats">
	{#if healthErrors.pipelines}
		<div
			...
			role="alert"
		>
			<p class="font-medium text-(--ii-warning)">Pipeline stats unavailable</p>
	...
	{:else}
		...
		{#if pipelineData.checked_at}
			<p class="mt-3 text-xs text-(--ii-text-muted)">
				Checked at
...
<SectionCard title="Workers">
	{#if healthErrors.workers}
		<div
			...
			role="alert"
		>
			<p class="font-medium text-(--ii-warning)">Worker status unavailable</p>
			<p class="mt-1">{healthErrors.workers}</p>
		</div>
	{:else if workerData.length > 0}
		<div class="mb-4 flex flex-wrap items-center gap-3">
			<div class="flex items-center gap-2 text-sm text-(--ii-text-secondary)">
				<span class="font-medium text-(--ii-text-primary)">Status</span>
				<Select
		...
		>
			{#snippet expandedRow(row)}
				...
				<div class="grid grid-cols-2 gap-4 px-4 py-3 text-sm sm:grid-cols-3">
					<div>
						<p class="text-xs text-(--ii-text-muted)">Worker</p>
						<p class="font-mono font-medium text-(--ii-text-primary)">{worker.name}</p>
					</div>
					<div>
						<p class="text-xs text-(--ii-text-muted)">Status</p>
						<p class="text-(--ii-text-primary)">{worker.status}</p>
					</div>
					<div>
						<p class="text-xs text-(--ii-text-muted)">Error Count</p>
						<p class="{worker.error_count > 0 ? "text-(--ii-danger)" : "text-(--ii-text-primary)"}">{worker.error_count}</p>
					</div>
					<div>
						<p class="text-xs text-(--ii-text-muted)">Last Run</p>
						<p class="text-(--ii-text-primary)">{worker.last_run ? formatDateTime(worker.last_run, "en-US") : "Never"}</p>
					</div>
					<div>
						<p class="text-xs text-(--ii-text-muted)">Checked At</p>
						<p class="text-(--ii-text-primary)">{worker.checked_at ? formatDateTime(worker.checked_at, "en-US") : "—"}</p>
					</div>
					<div>
						<p class="text-xs text-(--ii-text-muted)">Duration</p>
						<p class="text-(--ii-text-primary)">{worker.duration_ms !== null ? `${formatNumber(worker.duration_ms, 0, "en-US")}ms` : "—"}</p>
					</div>
					{#if worker.error_count > 0}
						<div class="col-span-2 sm:col-span-3">
							<p class="text-xs text-(--ii-text-muted)">Note</p>
							<p class="text-xs text-(--ii-warning)">This worker has recorded errors. Check the log feed below for details.</p>
						</div>
					{/if}
					{#if trigger}
						<div class="col-span-2 flex items-center gap-3 border-t border-(--ii-border) pt-3 sm:col-span-3">
							{#if trigger.name === "instrument_ingestion"}
								<label class="flex items-center gap-2 text-xs text-(--ii-text-secondary)">
									Lookback days
									<input
										type="number"
										min="1"
										...
										class="w-20 rounded-md border border-(--ii-border) bg-(--ii-surface) px-2 py-1 text-sm text-(--ii-text-primary)"
									/>
								</label>
							{/if}
							<Button
	...
	{:else}
		<p class="text-(--ii-text-muted)">No workers registered.</p>
	{/if}
</SectionCard>
