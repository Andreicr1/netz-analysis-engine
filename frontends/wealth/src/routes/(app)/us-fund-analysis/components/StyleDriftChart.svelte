<!-- Style Drift tab: stacked bar chart + drift signal table -->
<script lang="ts">
...
{#if !cik}
	<div class="sd-empty">
		<p>Select a manager from the Overview tab to view style drift.</p>
	</div>
{:else if loading}
...
{:else}
	...
	{#if chartOption}
		<div class="sd-chart">
			<ChartContainer option={chartOption} height={360} />
		</div>
	{:else}
					...
					{#each drift.drift_signals as sig (sig.sector)}
						<tr class="sd-row">
							...
							<td class="sd-td sd-td--right">
								<span
									class:sd-delta--up={sig.delta > 0}
									class:sd-delta--down={sig.delta < 0}
								>
							...
							<td class="sd-td">
								<span
									class="sd-signal"
									class:sd-signal--drift={sig.signal === "DRIFT"}
									class:sd-signal--stable={sig.signal === "STABLE"}
								>
									{sig.signal}
								</span>
							</td>
