<!-- Peer Compare tab: compare 2-5 managers side by side -->
<script lang="ts">
	...
	async function runCompare() {
		if (ciks.length < 2) {
			error = "Select at least 2 managers from the Overview tab (use checkboxes).";
			return;
		}
		loading = true;
		error = null;
		try {
			...
			ciks.forEach((c, i) => (params[`ciks`] = ciks.join(",")));
	...
	// Collect all sectors across all managers
...
<div class="pc-header">
	<div class="pc-status">
		{#if ciks.length === 0}
			<p class="pc-hint">Select managers from the Overview tab using checkboxes, then compare here.</p>
		{:else}
...
{:else if result}
		...
		{#each result.managers as mgr (mgr.cik)}
			<div class="pc-card">
				...
				<div class="pc-card__row">
					<span class="pc-card__label">CIK</span>
					<span class="pc-card__mono">{mgr.cik ?? "—"}</span>
				</div>
				<div class="pc-card__row">
					<span class="pc-card__label">AUM</span>
					<span>{mgr.aum_total != null ? formatCompact(mgr.aum_total) : "—"}</span>
				</div>
				<div class="pc-card__row">
					<span class="pc-card__label">Holdings</span>
					<span>{mgr.holdings_count}</span>
				</div>
				<div class="pc-card__row">
					<span class="pc-card__label">HHI</span>
					<span>
						{result.hhi_scores[mgr.cik ?? ""] != null
							? formatPercent(result.hhi_scores[mgr.cik ?? ""])
							: "—"}
					</span>
				</div>
			</div>
				...
				<tbody>
					{#each allSectors as sector}
						<tr class="pc-row">
							...
							{#each result.managers as mgr (mgr.cik)}
								<td class="pc-td pc-td--right">
									{formatPercent(result.sector_allocations[mgr.cik ?? ""]?.[sector] ?? 0)}
								</td>
							{/each}
						</tr>
					{/each}
	...
	<!-- Fund Structure Comparison (F-02) -->
	...
	{#if fundEntries.length > 0}
		...
		<div class="pc-fund-grid">
			{#each fundEntries as [cik, bd] (cik)}
				...
				<div class="pc-fund-card">
					<div class="pc-fund-card__name">
						{mgr?.firm_name?.slice(0, 25) ?? cik}
					</div>
		...
		<!-- Summary labels -->
		<div class="pc-fund-summary">
			{#each fundEntries as [cik, bd] (cik)}
				{@const mgr = result.managers.find((m) => m.cik === cik)}
				{@const top = bd.breakdown[0]}
				{#if top}
					<span class="pc-fund-summary__item">
						{mgr?.firm_name?.slice(0, 15) ?? cik}: {formatPercent(top.pct_of_total)} {top.fund_type}
					</span>
				{/if}
			{/each}
		</div>
