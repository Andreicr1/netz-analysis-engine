...
#### 4. **Session Historian** (foreground, after launching the above — only if the user opted in)
   ...
   - Dispatch in **foreground** — this agent reads session files outside the working directory (`~/.gemini/projects/`, `~/.codex/sessions/`, `~/.cursor/projects/`) which background agents may not have access to
