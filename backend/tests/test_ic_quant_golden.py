"""Golden-value tests for quant engine functions.

Captures exact outputs of build_sensitivity_2d(), build_sensitivity_3d_summary(),
and build_deterministic_scenarios() for known inputs BEFORE refactoring.
...
# ──────────────────────────────────────────────────────────────────
#  build_sensitivity_2d golden values
# ──────────────────────────────────────────────────────────────────


class TestSensitivity2DGolden:
    """Golden values for 2D sensitivity grid (default_rate x recovery_rate)."""

    def test_normal_base_return(self) -> None:
        """Normal case: base_return_pct=8.5%."""
        grid = build_sensitivity_2d(8.5, [])
        assert len(grid) == 16  # 4 default x 4 recovery
...
# ──────────────────────────────────────────────────────────────────
#  build_sensitivity_3d_summary golden values
# ──────────────────────────────────────────────────────────────────
...
class TestSensitivity3DGolden:
    ...
    def test_normal_3d_summary(self) -> None:
        ...
        # 3 shocks x 16 cells = 48 total cells
    ...
    def test_low_return_has_break_even(self) -> None:
        ...
        grid_2d = build_sensitivity_2d(3.0, [])
        summary = build_sensitivity_3d_summary(3.0, grid_2d)
    ...
    def test_high_return_no_break_even(self) -> None:
        ...
        grid_2d = build_sensitivity_2d(15.0, [])
        summary = build_sensitivity_3d_summary(15.0, grid_2d)
    ...
    def test_none_inputs(self) -> None:
        ...
        assert build_sensitivity_3d_summary(None, []) == {}
        assert build_sensitivity_3d_summary(8.5, []) == {}


# ──────────────────────────────────────────────────────────────────
#  build_deterministic_scenarios golden values
# ──────────────────────────────────────────────────────────────────
...
class TestScenariosGolden:
    ...
    def test_with_credit_metrics(self) -> None:
        ...
        assert ds["loss_rate_pct"] == 3.0  # from SCENARIO_PROXY
    ...
    def test_without_credit_metrics_proxy_mode(self) -> None:
        ...
        base = scenarios[0]
        assert base["loss_rate_pct"] == 1.0  # SCENARIO_PROXY["Base"]["loss_rate"]
    ...
    def test_liquidity_hooks(self) -> None:
        ...
        scenarios, _ = build_deterministic_scenarios(
            base_return_pct=8.5,
            risks=[],
