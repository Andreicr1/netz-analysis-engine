"""Consolidated Screener Views (Performance Layer)

Revision ID: 0078_consolidated_screener_views
Revises: 0077_dd_report_storage_path
...
revision: str = "0078_consolidated_screener_views"
down_revision: str | None = "0077_dd_report_storage_path"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

def upgrade() -> None:
    # 1. Create mv_unified_funds
    op.execute("""
    ...
    # 2. Create mv_unified_assets (Search Index)
    op.execute("""
        CREATE MATERIALIZED VIEW mv_unified_assets AS
        SELECT 
            instrument_id::text as id,
            name::text as name,
            ticker::text as ticker,
            isin::text as isin,
            asset_class::text as asset_class,
            'internal'::text as source,
            investment_geography::text as geography
        ...
        SELECT
            isin::text as id,
            fund_name::text as name,
            yahoo_ticker::text as ticker,
            isin::text as isin,
            'fund'::text as asset_class,
            'esma'::text as source,
            CASE 
                WHEN fund_name ILIKE '%emerging%' THEN 'Emerging Markets'
                WHEN fund_name ILIKE '%frontier%' THEN 'Emerging Markets'
                WHEN fund_name ILIKE '%china%' THEN 'Emerging Markets'
                WHEN fund_name ILIKE '%india%' THEN 'Emerging Markets'
                WHEN fund_name ILIKE '%latin america%' THEN 'Latin America'
                WHEN fund_name ILIKE '%latam%' THEN 'Latin America'
                WHEN fund_name ILIKE '%brazil%' THEN 'Latin America'
                WHEN fund_name ILIKE '%europe%' THEN 'Europe'
                WHEN fund_name ILIKE '%european%' THEN 'Europe'
                WHEN fund_name ILIKE '%eurozone%' THEN 'Europe'
                WHEN fund_name ILIKE '%asia%' THEN 'Asia Pacific'
                WHEN fund_name ILIKE '%japan%' THEN 'Asia Pacific'
                WHEN fund_name ILIKE '%pacific%' THEN 'Asia Pacific'
                WHEN fund_name ILIKE '%global%' THEN 'Global'
                WHEN fund_name ILIKE '%world%' THEN 'Global'
                WHEN fund_name ILIKE '%international%' THEN 'Global'
                WHEN fund_name ILIKE '%foreign%' THEN 'Global'
                WHEN fund_name ILIKE '%ex-us%' THEN 'Global'
                WHEN fund_name ILIKE '%ex us%' THEN 'Global'
                ELSE 'Europe'
            END as geography
        FROM esma_funds
        ...
        SELECT
            cusip::text as id,
            issuer_name::text as name,
            ticker::text as ticker,
            NULL::text as isin,
            CASE 
                WHEN security_type = 'REIT' THEN 'real_estate'
                ELSE 'equity'
            END as asset_class,
            'sec'::text as source,
            'US'::text as geography
    ...
    op.execute("CREATE UNIQUE INDEX idx_mv_unified_assets_id ON mv_unified_assets (id, source);")
    op.execute("CREATE INDEX idx_mv_unified_assets_name ON mv_unified_assets (name);")
    op.execute("CREATE INDEX idx_mv_unified_assets_ticker ON mv_unified_assets (ticker);")
    op.execute("CREATE INDEX idx_mv_unified_assets_isin ON mv_unified_assets (isin);")

def downgrade() -> None:
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_unified_assets CASCADE;")
