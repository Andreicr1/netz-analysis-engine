"""Fix mv_unified_funds manager_name resolution.

Remove fund_name fallback — only use actual SEC adviser firm_name.
Adds manager_name index for sorting/filtering.

Revision ID: 0080_fix_mv_unified_funds_manager
Revises: 0079_macro_performance_layer
...
revision: str = "0080_fix_mv_unified_funds_manager"
down_revision: str | None = "0079_macro_performance_layer"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

def upgrade() -> None:
    # Drop and recreate mv_unified_funds with fixed manager_name
...
def downgrade() -> None:
    # Revert to 0078 version with COALESCE fallback
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_unified_funds CASCADE;")
