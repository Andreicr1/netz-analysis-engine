...
def dispatch_compliance_ingest(
    *,
    background_tasks: BackgroundTasks,
    fund_id: uuid.UUID,
    actor_id: str,
    ...
    if _use_service_bus():
        ...
        sb_job_id = send_to_topic("compliance-pipeline", payload, stage="compliance")
        logger.info("Compliance ingest dispatched via Service Bus job=%s", sb_job_id)
        return {
            "status": "queued",
            "dispatch": "service_bus",
            "sb_job_id": sb_job_id,
    ...
    def _run() -> None:
        try:
            from app.domains.credit.compliance.ingest.compliance_ingest_runner import (
                ComplianceIngestRunner,
            )
            runner = ComplianceIngestRunner()
            runner.run(fund_id=str(fund_id))
        except Exception:
            logger.error("Compliance ingest failed fund=%s", fund_id, exc_info=True)

    background_tasks.add_task(_run)
    logger.info("Compliance ingest dispatched via BackgroundTasks fund=%s", fund_id)
    return {
        "status": "scheduled",
        "dispatch": "background_tasks",
        "fund_id": str(fund_id),
