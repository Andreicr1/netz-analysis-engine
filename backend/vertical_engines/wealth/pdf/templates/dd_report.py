...
def _cover_page(data: DDReportPDFData, labels: dict[str, str], language: Language, total_pages: int) -> str:
    ...
    total_critic = sum(_ch_critic_iter(ch) for ch in chapters)

    meta = (
        f'<div style="display:flex;gap:16px;margin-top:12px;font-size:9px;color:#6B7FA8">'
        f"<span>{ch_count} chapters</span>"
        f"<span>{total_critic} critic iterations</span>"
    ...
    return (
        ...
        f"Netz Adversarial AI Engine</div>"
        # Confidence gauge
        f"{_confidence_gauge(data.confidence_score)}"
        # Decision badge
        f'<div style="margin:8px 0">{_decision_badge(data.decision_anchor)}</div>'
        # TOC
...
def _chapter_page(
    ...
    *,
    ...
    for ch in chapters_on_page:
        ...
        # Chapter header
        parts.append(
            f'<div style="border-left:3px solid #185FA5;padding-left:12px;margin-bottom:10px">'
        ...
        if critic_iter > 0:
            parts.append(
                f'<div style="font-size:8px;color:#6b7280;margin:4px 0">'
                f"Critic: {critic_iter} iteration(s) &middot; {_e(critic_status)}</div>"
            )
    ...
    if is_final:
        parts.append(
            f'<div style="margin-top:12px;padding:12px;background:#f9fafb;'
            f'border:0.5px solid #e5e7eb;border-radius:4px;'
            f'font-size:8px;line-height:1.6;color:#6B7280">'
            f'<div style="font-weight:700;font-size:9px;color:#374151;'
            f'margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em">'
            f"Important Disclosures</div>"
            f"<p>{_e(labels['dd_disclaimer'])}</p>"
            f'<p style="margin-top:6px">This report was generated by an AI-powered '
            f"analytical engine and is intended solely for use by qualified institutional "
            f"investors. The content does not constitute investment advice. All analyses "
            f"are derived from publicly available sources and proprietary quantitative "
            f"models. Past performance is not indicative of future results.</p>"
            f'<p style="margin-top:6px">Confidence scoring methodology: chapters are '
            f"independently evaluated by an adversarial critic engine with circuit-breaker "
            f"controls. The overall confidence score is a weighted average of chapter-level "
            f"assessments. Scores below 60% trigger mandatory human review.</p>"
            f'<p style="margin-top:6px">&copy; Netz Analytics. All rights reserved.</p>'
            f"</div>"
        )
