...
```
        ...
        │  Pure math, zero I/O     │  │ Domain orchestrators      │
...
**Key invariants:**
- `quant_engine/` services are **pure sync computation** — no I/O, no DB, no ORM. Input: numpy arrays. Output: frozen dataclasses. Reusable across routes, workers, and reports.
...
| `wealth_embedding` | 900_041 | Daily | OpenAI embeddings for 16 sources into `wealth_vector_chunks` |
