...
There are no known methodological limitations affecting the engine at this time. 

*Update (Post-Publication):* The five previously documented limitations surrounding mixed-provenance volatility models, regime classification lookbacks, survivorship bias in peer rankings, missing return rejection audit trails, and hardcoded Cornish-Fisher relaxation factors were fully remediated during the post-publication stabilisation sprint. The engine now exposes full auditability via `vol_model` and `data_quality_flags` columns, covers a full 10-year regime lookback via the HMM Markov classifier, and dynamically parameterises tail-risk constraints per tenant mandate.
