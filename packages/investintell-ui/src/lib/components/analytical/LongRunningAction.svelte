...
<div
	class={cn(
		"rounded-xl border border-(--ii-border) bg-(--ii-surface) p-4 shadow-sm",
		className,
	)}
>
	<div class="flex flex-wrap items-start justify-between gap-3">
		<div class="space-y-1">
			<p class="text-sm font-semibold text-(--ii-text-primary)">{title}</p>
			{#if description}
		...
		<div
			...
			style={`background-color: color-mix(in srgb, ${statusTone} 14%, var(--ii-surface)); color: ${statusTone};`}
		>
	...
	<div class="mt-4 space-y-3">
		<div class="space-y-1">
			<p class="text-sm text-(--ii-text-primary)">{statusMessage}</p>
			<div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-(--ii-text-muted)">
				{#if progressLabel}
		...
		<div class="h-2 overflow-hidden rounded-full bg-(--ii-surface-inset)">
			<div
		...
		{#if slaExceeded && (currentState === "starting" || currentState === "in-flight")}
			<div
				...
				style="border-color: var(--ii-warning); background-color: color-mix(in srgb, var(--ii-warning) 10%, var(--ii-surface)); color: var(--ii-text-primary);"
			>
				<p class="font-medium" style="color: var(--ii-warning);">{slaMessage}</p>
			</div>
		{/if}
		...
		{#if currentState === "error" && failureDetail}
			<div
				class="rounded-lg border px-3 py-2 text-sm"
				style="border-color: var(--ii-danger); background-color: color-mix(in srgb, var(--ii-danger) 10%, var(--ii-surface)); color: var(--ii-text-primary);"
			>
				<p class="font-medium text-(--ii-danger)">Failure detail</p>
				<p class="mt-1 text-(--ii-text-secondary)">{failureDetail}</p>
			</div>
		{/if}
	</div>
