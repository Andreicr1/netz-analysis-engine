	...
	function buildMergeTheme(EditorView: typeof import("@codemirror/view").EditorView) {
		return EditorView.theme({
			...
			".cm-deletedChunk .cm-deletedLine": {
				backgroundColor: "color-mix(in srgb, var(--ii-danger) 12%, transparent)",
			},
			".cm-insertedChunk .cm-insertedLine": {
				backgroundColor: "color-mix(in srgb, var(--ii-success) 12%, transparent)",
			},
			".cm-changedText": {
				backgroundColor: "color-mix(in srgb, var(--ii-warning) 20%, transparent)",
			},
			".cm-mergeGutter": {
				backgroundColor: "var(--ii-surface-alt)",
			},
...
<SectionCard title="Diff — {diff.config_type}" class={className}>
	<div class="space-y-4">
		...
		<div class="flex items-center justify-between">
				...
				{#if diff.changed_keys.length > 0}
					<div class="flex flex-wrap gap-1">
						{#each diff.changed_keys as key (key)}
							<span
								class="rounded-full border border-(--ii-border) bg-(--ii-surface-alt) px-2 py-0.5 font-mono text-xs text-(--ii-text-secondary)"
							>
								{key}
							</span>
						{/each}
			...
			<div class="flex shrink-0 gap-1 rounded-md border border-(--ii-border) p-0.5">
				<button
					onclick={() => (viewMode = "split")}
					class="rounded px-3 py-1 text-xs font-medium transition-colors {viewMode === 'split'
						? 'bg-(--ii-brand-primary) text-white'
						: 'text-(--ii-text-secondary) hover:bg-(--ii-surface-alt)'}"
				>
					Split
				</button>
				<button
		...
		<div
			bind:this={mergeHost}
			class="overflow-hidden rounded-xl border border-(--ii-border) bg-(--ii-surface)"
