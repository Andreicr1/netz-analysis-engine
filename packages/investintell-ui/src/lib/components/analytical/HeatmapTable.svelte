<script lang="ts">
	...
	function cellStyle(value: number): string {
		...
		if (t >= 0.5) {
			...
			return `background-color: color-mix(in srgb, var(--ii-danger) ${Math.round(intensity * 60)}%, var(--ii-surface-inset));`;
		} else {
			const intensity = (0.5 - t) * 2; // 0→1 for lower half
			return `background-color: color-mix(in srgb, var(--ii-success) ${Math.round(intensity * 60)}%, var(--ii-surface-inset));`;
		}
	}
	...
	<table class="w-full border-collapse text-xs">
		<thead>
			<tr>
				<!-- Corner cell -->
				<th
					class="min-w-[120px] px-3 py-2 text-left text-(--ii-text-muted)"
					scope="col"
				></th>
				{#each columns as col (col)}
					<th
						class="min-w-[72px] px-2 py-2 text-center font-medium text-(--ii-text-secondary)"
						scope="col"
					>
						{col}
					</th>
		...
		<tbody>
			{#each rows as row, ri (row)}
				<tr class="border-t border-(--ii-border)">
					<th
						class="px-3 py-2 text-left font-medium text-(--ii-text-secondary)"
						scope="row"
					>
						{row}
					</th>
