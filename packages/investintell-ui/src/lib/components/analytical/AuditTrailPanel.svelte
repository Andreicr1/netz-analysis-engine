...
<Card class={cn("p-5", className)}>
	<div class="space-y-5">
		<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
			<div class="space-y-1">
				<h2 id={getTitleId(title)} class="text-lg font-semibold text-(--ii-text-primary)">
					{title}
				</h2>
				<p class="text-sm text-(--ii-text-secondary)">{description}</p>
			</div>
		...
		{#if entries.length === 0}
			<div class="rounded-lg border border-dashed border-(--ii-border) p-4 text-sm text-(--ii-text-secondary)">
				{emptyMessage}
			</div>
		{:else}
			<ul
			...
			>
				{#each getVisibleEntries(entries, showAll, maxVisible) as row (row.key)}
					{#if row.groupLabel}
						<li class="pt-2">
							<h3
								class="text-xs font-semibold uppercase tracking-[0.18em] text-(--ii-text-secondary)"
							>
								{row.groupLabel}
							</h3>
						</li>
					...
					<li class="rounded-lg border border-(--ii-border) bg-(--ii-surface-alt) p-4">
						{#if entryRenderer}
							{@render entryRenderer(row.entry)}
						{:else}
						<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
							<div class="space-y-1">
								<p class="text-sm font-semibold text-(--ii-text-primary)">
									{row.entry.action}
								</p>
								<p class="text-sm text-(--ii-text-secondary)">{row.entry.scope}</p>
							</div>

							<div class="flex flex-wrap items-center justify-start gap-2 sm:justify-end">
								{#if row.entry.immutable}
									<span
										class="inline-flex w-fit items-center rounded-full border border-(--ii-border) bg-(--ii-surface) px-2.5 py-1 text-xs font-medium text-(--ii-text-secondary)"
									>
										Immutable
									</span>
								{/if}
						...
						<dl class="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
							<div>
								<dt class="text-xs font-medium uppercase tracking-[0.14em] text-(--ii-text-secondary)">
									Actor
								</dt>
								<dd class="mt-1 text-(--ii-text-primary)">{row.entry.actor}</dd>
							</div>
							...
							{#if row.entry.actorCapacity}
								<div>
									...
									<dd class="mt-1 text-(--ii-text-primary)">{row.entry.actorCapacity}</dd>
								</div>
							{/if}
							...
							{#if row.entry.actorEmail}
								<div>
									...
									<dd class="mt-1 text-(--ii-text-primary)">{row.entry.actorEmail}</dd>
								</div>
							{/if}
							...
							<div>
								...
								<dd class="mt-1 text-(--ii-text-primary)">
							...
							{#if row.entry.sourceSystem}
								<div>
									...
									<dd class="mt-1 text-(--ii-text-primary)">{row.entry.sourceSystem}</dd>
								</div>
							{/if}
						</dl>

						{#if row.entry.rationale}
							<div class="mt-4 rounded-md border border-(--ii-border) bg-(--ii-surface) p-3">
								<p class="text-xs font-medium uppercase tracking-[0.14em] text-(--ii-text-secondary)">
									Rationale
								</p>
								<p class="mt-2 text-sm leading-6 text-(--ii-text-primary)">
									{row.entry.rationale}
								</p>
							</div>
						{/if}
							...
							<div class="mt-4 space-y-3">
								...
								<ul class="space-y-2">
									{#each row.entry.changedFields as change (change.field)}
										<li class="rounded-md border border-(--ii-border) bg-(--ii-surface) p-3">
											<p class="text-sm font-medium text-(--ii-text-primary)">{change.field}</p>
											<div class="mt-2 grid gap-2 text-sm sm:grid-cols-2">
												<div>
													<p class="text-xs uppercase tracking-[0.12em] text-(--ii-text-secondary)">
														From
													</p>
													<p class="mt-1 text-(--ii-text-primary)">{change.from ?? "Not set"}</p>
												</div>
												<div>
													<p class="text-xs uppercase tracking-[0.12em] text-(--ii-text-secondary)">
														To
													</p>
													<p class="mt-1 text-(--ii-text-primary)">{change.to ?? "Not set"}</p>
												</div>
											</div>
										</li>
									{/each}
