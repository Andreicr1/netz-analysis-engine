...
<div class={cn("flex flex-col divide-y divide-(--ii-border)", className)} role="feed">
	{#if visible.length === 0}
		<p class="py-8 text-center text-sm text-(--ii-text-muted)">Nenhum alerta.</p>
	{:else}
			...
			>
				<div class="min-w-0 flex-1">
					...
					{:else if alert.type === "behavior_change"}
						<p class="mt-0.5 text-xs text-(--ii-text-secondary)">
				...
				<time
					class="shrink-0 text-xs text-(--ii-text-muted)"
