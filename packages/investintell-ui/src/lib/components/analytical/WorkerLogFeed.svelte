...
<script lang="ts">
	...
	> = {
		critical: { label: "Critical", tone: "var(--ii-danger)" },
		error: { label: "Error", tone: "var(--ii-danger)" },
		warning: { label: "Warning", tone: "var(--ii-warning)" },
		info: { label: "Info", tone: "var(--ii-info)" },
		debug: { label: "Debug", tone: "var(--ii-text-secondary)" },
	};
...
<div class="space-y-3">
	<div class="flex flex-wrap items-center gap-2">
		<div class="flex items-center gap-2 rounded-full border border-(--ii-border) bg-(--ii-surface) px-3 py-1 text-xs text-(--ii-text-secondary)">
		...
		<button
			...
			class="rounded-md border border-(--ii-border) bg-(--ii-surface) px-3 py-1 text-xs text-(--ii-text-primary) hover:bg-(--ii-surface-alt)"
		>
			{isLive ? "Disconnect" : "Reconnect"}
		</button>

		{#if connection?.error}
			<p class="text-xs text-(--ii-danger)">{connection.error.message}</p>
		{/if}
	</div>
	...
	<div class="flex flex-wrap items-center gap-2">
				...
				<button
					...
					class={`rounded-full border px-3 py-1 text-xs transition ${
						severityFilter === severity
							? "border-(--ii-brand-primary) bg-(--ii-brand-primary) text-white"
							: "border-(--ii-border) bg-(--ii-surface) text-(--ii-text-secondary) hover:bg-(--ii-surface-alt)"
					}`}
				>
					{severity === "all"
						? `All (${logs.length})`
		...
		<input
			type="search"
			class="min-w-0 flex-1 rounded-md border border-(--ii-border) bg-(--ii-surface) px-3 py-2 text-sm text-(--ii-text-primary) placeholder:text-(--ii-text-muted) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ii-brand-secondary)"
	...
	<div
		...
		class="max-h-[28rem] overflow-auto rounded-xl border border-(--ii-border) bg-(--ii-surface)"
		role="log"
		aria-live="polite"
	...
	>
					...
					<article class="border-l-4 px-4 py-3" style={`border-left-color: ${severityMeta[entry.severity].tone};`}>
						<div class="flex flex-wrap items-start gap-3">
							<span
								...
								style={`border-color: ${severityMeta[entry.severity].tone}; background-color: color-mix(in srgb, ${severityMeta[entry.severity].tone} 12%, var(--ii-surface)); color: ${severityMeta[entry.severity].tone};`}
							>
								{severityMeta[entry.severity].label}
							</span>
							<div class="min-w-0 flex-1">
								<p class="break-words font-mono text-xs text-(--ii-text-primary)">
									{entry.message}
								</p>
								<div class="mt-1 flex flex-wrap gap-3 text-[11px] text-(--ii-text-muted)">
									{#if entry.source}
		...
		{:else}
			<div class="px-4 py-8 text-sm text-(--ii-text-muted)">
