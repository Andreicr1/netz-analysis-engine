...
<div class={cn("w-full", className)}>
	{#if showValues}
		<div class="mb-1 flex justify-end gap-1 text-xs text-(--ii-text-muted)">
	...
	<div
		...
		style="background-color: var(--ii-surface-inset);"
		role="progressbar"
	...
	>
		...
		{#if ratio > 1}
			<div
				...
				style="left: {(1 / Math.min(ratio, 1.2)) * 100}%; background-color: var(--ii-text-muted); opacity: 0.5;"
			></div>
		{/if}
