...
<section
	class={cn(
		"ii-ui-surface overflow-hidden rounded-(--ii-radius-lg)",
		className,
	)}
>
	<!-- Header -->
	...
	<div
		class={cn(
			"flex items-center justify-between gap-4 bg-(--ii-surface-highlight) px-(--ii-space-card-padding-lg) py-[18px] transition-[background-color,box-shadow]",
			collapsible && "cursor-pointer select-none hover:bg-(--ii-accent-soft)",
		)}
	...
	{#if !collapsed}
		...
		>
			...
			{#if loading}
				...
				>
					<div
						class="h-4 w-3/4 animate-pulse rounded bg-(--ii-surface-inset)"
					></div>
					<div
						class="h-4 w-1/2 animate-pulse rounded bg-(--ii-surface-inset)"
					></div>
					<div
						class="h-4 w-2/3 animate-pulse rounded bg-(--ii-surface-inset)"
					></div>
				</div>
			{/if}
