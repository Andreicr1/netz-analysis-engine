<script lang="ts">
	...
	const trendColors: Record<Trend, string> = {
		up: "text-(--ii-success)",
		down: "text-(--ii-danger)",
		flat: "text-(--ii-text-muted)",
	};
</script>

<div
	class={cn(
		"rounded-lg border border-(--ii-border) bg-(--ii-surface) p-4 shadow-sm",
		className,
	)}
>
	<div class="flex items-start justify-between">
		<div>
			<p class="text-sm text-(--ii-text-muted)">{label}</p>
			<p class="mt-1 text-2xl font-semibold text-(--ii-text-primary)">
				{value}
			</p>
			{#if trend}
