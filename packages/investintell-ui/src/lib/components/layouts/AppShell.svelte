...
<div
	class={cn("ii-app-shell", className)}
	style:--sidebar-width={sidebarWidth}
	style:--panel-width={panelWidth}
>
	{#if sidebar}
		<aside class="ii-app-shell__sidebar" class:collapsed={sidebarCollapsed}>
			{@render sidebar()}
		</aside>
	{/if}

	<main class="ii-app-shell__main">
		{#if main}
			{@render main()}
		{:else if children}
			{@render children()}
...
<style>
	...
	@media (max-width: 599px) {
		...
		.ii-app-shell__sidebar:not(.collapsed) {
			transform: translateX(0);
		}
		.ii-app-shell__panel {
			width: 100%;
